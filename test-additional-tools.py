...
def main():
    ...
    try:
        ...
        # Initialize
        send_message(proc, 1, "initialize", {
        ...
        response = send_message(proc, 2, "tools/call", {
            "name": "get_listing",
            "arguments": {
        ...
        response = send_message(proc, 3, "tools/call", {
            "name": "update_listing",
            "arguments": {
                "listing_id": EXISTING_LISTING_ID,
        ...
        print("NOTE: Skipping register tool test as it would create another account", file=sys.stderr)
        
        # Test the ACE-T trust system tools (these might not have endpoints implemented yet)
        print("\n=== Test register_agent ===", file=sys.stderr)
        response = send_message(proc, 4, "tools/call", {
            "name": "register_agent",
            "arguments": {
        ...
        if response and "result" in response and not response.get("result", {}).get("isError"):
            print("register_agent: SUCCESS", file=sys.stderr)
        else:
            print(f"register_agent: Expected to fail (not implemented) - {response}", file=sys.stderr)
        
        # Test query_trust 
        print("\n=== Test query_trust ===", file=sys.stderr)
        response = send_message(proc, 5, "tools/call", {
            "name": "query_trust",
            "arguments": {
        ...
        if response and "result" in response and not response.get("result", {}).get("isError"):
            ...
            print(f"query_trust: Expected to fail (not implemented) - {response}", file=sys.stderr)
        
        # Test search_agents
        print("\n=== Test search_agents ===", file=sys.stderr)
        response = send_message(proc, 6, "tools/call", {
            "name": "search_agents",
            "arguments": {
                "min_score": 0.0,
                "limit": 10
        ...
        if response and "result" in response and not response.get("result", {}).get("isError"):
            ...
            print(f"search_agents: Expected to fail (not implemented) - {response}", file=sys.stderr)

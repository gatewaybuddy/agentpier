...
class AgentPierError(Exception):
    ...
    def __init__(self, message: str, error_code: Optional[str] = None, status_code: Optional[int] = None, details: Optional[str] = None):
...
class APIError(AgentPierError):
    ...
    def __init__(self, response_data: Dict[str, Any], status_code: int):
        error_msg = response_data.get('message', response_data.get('error', 'Unknown API error'))
...
def raise_for_status(response, response_data: Optional[Dict[str, Any]] = None):
    ...
    if not error_data:
        try:
            ...
            error_data = {'error': 'Unknown error', 'message': response.text or f'HTTP {status_code}'}
    
    message = error_data.get('message', error_data.get('error', f'HTTP {status_code} error'))
    ...
    if status_code == 400:
        ...
        raise RateLimitError(message, retry_after, error_code=error_code, status_code=status_code, details=details)

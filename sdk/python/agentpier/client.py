...
from .exceptions import AgentPierError, NetworkError, raise_for_status
...
class AgentPierClient:
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key.startswith("ap_"):
            raise AgentPierError(
                "API key must start with 'ap_' prefix (e.g., 'ap_live_xxx' or 'ap_test_xxx')"
            )
        ...
        if api_key.startswith("ap_live_") or api_key.startswith("ap_test_"):
            ...
            raise AgentPierError(
                "API key must start with 'ap_live_' or 'ap_test_' prefix"
            )
    ...
    def request(
        ...
        **kwargs,
        ...
        # Request kwargs
        req_kwargs = {"timeout": self.timeout, "headers": req_headers, **kwargs}

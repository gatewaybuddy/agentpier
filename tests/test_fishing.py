...
class TestCastLine:
    def test_successful_cast(self, dynamodb, sample_user):
        ...
        with patch('handlers.fishing.random.randint') as mock_randint, \
             patch('handlers.fishing.random.choice') as mock_choice, \
             patch('handlers.fishing.random.uniform') as mock_uniform:
    ...
    def test_rate_limiting(self, dynamodb, sample_user):
        ...
        assert ("m " in body["message"] or "s " in body["message"] or 
                "minutes" in body["message"] or "seconds" in body["message"])
    ...
    def test_all_rarities_possible(self, dynamodb, sample_user):
        ...
        for i, (roll, expected_rarity) in enumerate(test_cases):
            with patch('handlers.fishing.random.randint', return_value=roll), \
                 patch('handlers.fishing.random.choice', side_effect=lambda x: x[0]), \
                 patch('handlers.fishing.random.uniform', return_value=1.0), \
                 patch('handlers.fishing._check_cast_cooldown', return_value=(True, 0)):  # Bypass cooldown
...
class TestTackleBox:
    ...
    def test_tackle_box_pagination(self, dynamodb, sample_user):
        ...
        _, raw_key = sample_user
        event = make_api_event(method="GET", api_key=raw_key, query_params={"limit": "5"})
        resp = get_tackle_box(event, None)

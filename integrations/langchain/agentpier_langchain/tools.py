...
class AgentVerificationInput(BaseModel):
    ...
    min_score: Optional[float] = Field(default=60.0, description="Minimum trust score required (0-100)")
...
"""
            ...
            if moltbook_data:
                verification_status = '✓ Verified' if moltbook_data.get('verified') else '✗ Not verified'
...
class AgentVerificationTool(BaseTool):
    ...
    def _run(self, agent_ids: str, min_score: float = 60.0) -> str:
        ...
        for agent_id in agent_list:
            try:
                ...
                if trust_score >= min_score:
                    results.append(f"✅ {agent_name}: {trust_score:.1f}/100 ({trust_tier}) - PASSED")
                else:
                    results.append(f"❌ {agent_name}: {trust_score:.1f}/100 ({trust_tier}) - FAILED (required: {min_score})")
                    all_passed = False
        ...
        recommendation = "Safe to proceed with workflow" if all_passed else "Review failed agents before proceeding"
    ...
    def call(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "moltbook_verified": data.get('sources', {}).get('moltbook', {}).get('verified', False),

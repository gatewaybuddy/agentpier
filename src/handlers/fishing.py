...
def _get_user_catch_stats(user_id, table):
    ...
    for catch in catches:
        ...
        if rarest_catch is None or rarity_order.get(rarity, 0) > rarity_order.get(
            rarest_catch.get("rarity", "common"), 0
        ):
            rarest_catch = catch
...
def get_leaderboard(event, context):
    ...
    if leaderboard_type == "biggest":
        ...
        sorted_anglers = sorted(
            catch_counts.values(), key=lambda x: x["count"], reverse=True
        )[:10]
        ...
        return error(
            "Invalid leaderboard type. Use: biggest, recent, or most", "invalid_type"
        )
...
def get_tackle_box(event, context):
    ...
    result = {
        ...
        "pagination": {"limit": limit, "has_more": len(catches) == limit},
    }
...
def get_pier_stats(event, context):
    ...
    response = table.query(
        IndexName="GSI2", KeyConditionExpression=Key("GSI2PK").eq("PIER#LEADERBOARD")
    )
    ...
    rarest_catch = max(
        catches, key=lambda x: rarity_order.get(x.get("rarity", "common"), 0)
    )
    ...
    result = {
        ...
        "most_active_angler": (
            {"username": most_active["username"], "total_catches": most_active["count"]}
            if most_active
            else None
        ),
...
def _get_pier_status_message(total_catches, legendary_count):
    ...
    if legendary_count > 5:
        ...
        return (
            "📈 Business is good! Agents are finding success in these digital waters."
        )
    elif total_catches > 10:
        return (
            "🌊 The pier is getting popular. Word is spreading about the good fishing!"
        )
    else:
        return (
            "🏁 Early days at the pier. Perfect time to establish your fishing legacy!"
        )

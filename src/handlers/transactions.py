...
def create_transaction(event, context):
    ...
    transaction = {
        ...
        "status": "pending",
        "amount": Decimal(str(body.get("amount"))) if body.get("amount") is not None else None,
...
def update_transaction(event, context):
    ...
    if current_status == "completed":
        return error("Cannot change status of completed transaction", "invalid_transition")
    
    if current_status == "cancelled":
        return error("Cannot change status of cancelled transaction", "invalid_transition")

    # Role-based permissions
    if new_status == "completed":
        # Only provider can mark as completed
        if user_id != provider_id:
            return error("Only the provider can mark transaction as completed", "forbidden", 403)
    
    elif new_status == "disputed":
        # Only consumer can dispute
        if user_id != consumer_id:
            return error("Only the consumer can dispute a transaction", "forbidden", 403)
        # Can only dispute pending or completed transactions
        if current_status not in ("pending", "completed"):
            return error("Can only dispute pending or completed transactions", "invalid_transition")
...
def create_review(event, context):
    ...
    if existing_review_resp.get("Item"):
        return error("You have already reviewed this transaction", "already_reviewed", 409)
